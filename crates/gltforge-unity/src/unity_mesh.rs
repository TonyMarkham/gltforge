...
/// `ptr` must be a valid, non-null handle. `out_len` may be null.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn gltforge_mesh_tangents(
    ptr: *const UnityGltf,
    mesh_idx: u32,
    out_len: *mut u32,
) -> *const f32 {
    let gltf = unsafe { &*ptr };
    match gltf.meshes.get(&mesh_idx) {
        Some(m) if !m.tangents.is_empty() => {
            let flat = m.tangents.as_flattened();
            if !out_len.is_null() {
                unsafe { *out_len = flat.len() as u32 };
            }
            flat.as_ptr()
...
/// `ptr` must be a valid, non-null handle.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn gltforge_mesh_uv_channel_count(
    ptr: *const UnityGltf,
    mesh_idx: u32,
) -> u32 {
    unsafe { &*ptr }
...
/// `ptr` must be a valid, non-null handle. `out_len` may be null.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn gltforge_mesh_uvs(
    ptr: *const UnityGltf,
    mesh_idx: u32,
    channel: u32,
    out_len: *mut u32,
    ...
    match gltf
        .meshes
        .get(&mesh_idx)
        .and_then(|m| m.uvs.get(channel as usize))
    {
        Some(ch) if !ch.is_empty() => {
            let flat = ch.as_flattened();
            if !out_len.is_null() {
                unsafe { *out_len = flat.len() as u32 };
            }
            flat.as_ptr()
